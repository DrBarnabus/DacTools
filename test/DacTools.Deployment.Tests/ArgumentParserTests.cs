...
public class ArgumentParserTests
{
    ...
    public static IEnumerable<object[]> InvalidThreadsTestData(string switchStart)
    {
        return new List<object[]>
        {
            ...
            {
                $"{switchStart} 0",
                "Threads parameter must be either minus one or greater than zero. Parsed a value of '0'."
            },
            new object[]
            {
                $"{switchStart} -2",
                "Threads parameter must be either minus one or greater than zero. Parsed a value of '-2'."
            },
    ...
    public static IEnumerable<object[]> CorrectDacDeployOptionsDoNotDropObjectTypesValues(string switchStart)
    {
        return new List<object[]>
        {
            new object[]
            {
                ...
                new[]
                {
                    ObjectType.Filegroups, ObjectType.Files, ObjectType.Logins, ObjectType.Permissions,
                    ObjectType.RoleMembership, ObjectType.Users
                }
            },
    ...
    [MemberData(nameof(CorrectDacDeployOptionsDoNotDropObjectTypesValues), "-p:DoNotDropObjectTypes")]
    public void ShouldSetTheCorrectDacDeployOptionsDoNotDropObjectTypesValues(string arguments,
        ObjectType[] expectedValues)
    {
        var argumentParser = new ArgumentParser();
        var result = argumentParser.ParseArguments(arguments);
        result.IsHelp.ShouldBeFalse();
    ...
    [InlineData("-variable:CustomSqlCmdVariable VariableValue", "CustomSqlCmdVariable", "VariableValue")]
    public void ShouldSetTheCorrectSqlCommandVariableValue(string arguments, string expectedVariableName,
        string expectedVariableValue)
    {
        ...
        result.Threads.ShouldBe(0);
        result.DatabaseNames.ShouldBeEmpty();
        result.DacDeployOptions.SqlCommandVariableValues.ShouldContainKeyAndValue(expectedVariableName,
    ...
    public static IEnumerable<object[]> CorrectSqlCommandVariableValuesWhenMultipleSupplied(string switchStart)
    {
        return new List<object[]>
        {
            new object[]
            {
                $"{switchStart}variable:Variable1 Value1 {switchStart}variable:Variable2 Value2",
                new[] { "Variable1", "Variable2" }, new[] { "Value1", "Value2" }
            }
        };
    }
    ...
    [MemberData(nameof(CorrectSqlCommandVariableValuesWhenMultipleSupplied), "-")]
    public void ShouldSetTheCorrectSqlCommandVariableValuesWhenMultipleSupplied(string arguments,
        string[] expectedVariableNames, string[] expectedVariableValues)
    {
        var argumentParser = new ArgumentParser();
        var result = argumentParser.ParseArguments(arguments);
        result.IsHelp.ShouldBeFalse();
        ...
        for (int i = 0; i < expectedVariableNames.Length; i++)
            result.DacDeployOptions.SqlCommandVariableValues.ShouldContainKeyAndValue(expectedVariableNames[i],
                expectedVariableValues[i]);
    }
